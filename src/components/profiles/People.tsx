...
class People extends React.Component<MyState, MyProps> {
  constructor(props) {
    super(props);
  ...
  render = () => {
    let people = this.state.people.map((person, index) => (
      <tr key={index}>
        ...
        <td>
          <Link
            to={`/people/${person.id}?name=${person.name}&occupation=${person.occupation}`}
          >
            {person.name}
          </Link>
        </td>
        ...
        <td>
          <Button onClick={(e) => this.onDeletePerson(e, person.id)}>
            Delete
          </Button>
        </td>
      </tr>
